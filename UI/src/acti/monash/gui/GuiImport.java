...
public class GuiImport extends JPanel
{
	...
	public static void launch()
	{
		EventQueue.invokeLater(new Runnable()
		{
			...
			public void run()
			{
				try
				{
					GuiFrame.frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
					GuiFrame.frame.setSize(GuiFrame.frame.getMinimumSize().width, 600);
					GuiImport guiImport = new GuiImport();
					// GuiFrame.frame.addToFrame(new GuiMain());
					GuiFrame.frame.addToFrame(guiImport);
	...
	public static void copyFile(File file)
	{
		...
		try
		{
			...
			// file.getName().length())));
			output = new FileOutputStream(new File(PythonRunner.pythonLocation + "data.csv"));
